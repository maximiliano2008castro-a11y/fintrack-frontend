...
const CajaFuerteFullScreen = ({ 
    ...
    handleManualMeta, isDarkMode // 💡 Prop agregado
}) => {
        ...
        <div className={isDarkMode ? 'theme-dark' : 'theme-light'} style={fullScreenStyle}>
                ...
                <div style={overlayStyle}>
                    <div style={modalCenterStyle}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                            <div style={{...badgeDayStyle, background: 'var(--primary)'}}>Día {selectedDay} (Metas)</div>
                        ...
                        <div style={typeSelectorGrid}>
                            <button onClick={() => setEventForm({...eventForm, tipo: 'pago'})} style={{ ...typeBtn, border: eventForm.tipo === 'pago' ? '2px solid var(--success)' : '1px solid var(--border-color)', background: eventForm.tipo === 'pago' ? 'var(--success-light)' : 'var(--bg-secondary)', color: eventForm.tipo === 'pago' ? 'var(--success)' : 'var(--text-main)' }}>Abono Proyectado</button>
                            <button onClick={() => setEventForm({...eventForm, tipo: 'gasto'})} style={{ ...typeBtn, border: eventForm.tipo === 'gasto' ? '2px solid var(--danger)' : '1px solid var(--border-color)', background: eventForm.tipo === 'gasto' ? 'var(--danger-light)' : 'var(--bg-secondary)', color: eventForm.tipo === 'gasto' ? 'var(--danger)' : 'var(--text-main)' }}>Compra de Meta</button>
                        </div>
                        ...
                        <button onClick={handleSaveLocalEvent} style={{ ...btnMainAdd, width: '100%', background: 'var(--primary)' }}>Programar Alerta</button>
                    </div>
                </div>
            )}
            ...
            <div className="header-responsive" style={headerStyle}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '20px', width: '100%', maxWidth: '1200px', margin: '0 auto' }}>
                    ...
                    <h1 className="header-title-responsive" style={{ margin: '0', color: 'var(--primary)', fontSize: '28px', display: 'flex', alignItems: 'center', gap: '10px' }}><FaLock /> Caja Fuerte (Ahorros)</h1>
                </div>
            </div>
            ...
            <div className="content-responsive" style={contentContainer}>
                ...
                {/* TARJETA GRAN TOTAL */}
                <div className="acumulado-card-responsive" style={{...acumuladoCard, background: 'var(--primary)'}}>
                    <p style={{ margin: '0 0 5px 0', fontSize: '18px', color: 'rgba(255,255,255,0.9)', fontWeight: 'bold' }}>Gran Total (Libre + Metas)</p>
                    <h1 className="acumulado-total-responsive" style={{ margin: 0, fontSize: '64px', color: '#fff', textShadow: '0 0 15px rgba(255,255,255,0.3)' }}>${granTotal.toLocaleString('es-MX', { minimumFractionDigits: 2 })}</h1>
                </div>
                ...
                <div className="grid-responsive" style={gridStyle}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
                        <div>
                            <h3 style={sectionTitle}><FaLock color="var(--primary)" /> Ahorro General (Libre)</h3>
                            <div style={retoCard}>
                                <h2 style={{ fontSize: '42px', margin: '0 0 20px 0', color: 'var(--primary)', textAlign: 'center' }}>${saldoCajaFuerte.toLocaleString('es-MX', { minimumFractionDigits: 2 })}</h2>
                                ...
                                <div style={{ display: 'flex', gap: '10px', marginBottom: '15px' }}>
                                    ...
                                    <button onClick={handleAdd} style={{...btnAccion, background: 'var(--success-alt)'}}><FaPlus /> Ingresar</button>
                                </div>
                                ...
                                <div style={{ display: 'flex', gap: '10px' }}>
                                    ...
                                    <button onClick={handleWithdraw} style={{...btnAccion, background: 'var(--danger-alt)'}}><FaMinus /> Retirar</button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            <h3 style={sectionTitle}><FaHistory color="var(--primary)" /> Historial de Ahorros</h3>
                            <div style={historialContainer}>
                                {historialCaja.length === 0 ? <p style={{textAlign:'center', color:'var(--text-light)', fontSize:'13px', padding: '20px'}}>No hay movimientos en tus metas ni en tu caja.</p> : 
                                    historialCaja.map(h => {
                                        ...
                                        return (
                                            <div key={h.id} style={historialItem}>
                                                ...
                                                <span style={{ fontWeight: 'bold', fontSize: '14px', color: isIngreso ? 'var(--success)' : 'var(--danger)' }}>{isIngreso ? '+' : '-'}${h.monto}</span>
                                            </div>
                                        )
                                    })
                                }
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
                        <div>
                            <h3 style={sectionTitle}><FaBrain color="var(--primary)" /> Metas de Ahorro Inteligente</h3>
                            {metasActivas.length === 0 ? (
                                <p style={{ color: 'var(--text-muted)', background: 'var(--bg-card)', padding: '30px', borderRadius: '20px', border: '1px dashed var(--primary)', textAlign: 'center' }}>No tienes Ahorros Inteligentes en progreso. ¡Créalos en el Laboratorio!</p>
                            ) : (
                                        ...
                                        return (
                                            <div key={meta} style={retoCard}>
                                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px' }}>
                                                    <strong style={{ fontSize: '18px', color: 'var(--text-main)' }}>{meta.replace(/ \(Total: [\d.]+\)/, '').replace('Meta: ', '')}</strong>
                                                    {total > 0 && <span style={{ fontSize: '14px', color: 'var(--primary)', fontWeight: 'bold' }}>Meta: ${total.toLocaleString()}</span>}
                                                </div>
                                                
                                                <div style={progressBg}><div style={{...progressFill, width: `${porcentaje}%`, boxShadow: isDarkMode ? '0 0 10px rgba(59, 130, 246, 0.8)' : 'none'}}></div></div>
                                                
                                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '15px', flexWrap: 'wrap' }}>
                                                    <span style={{ fontSize: '15px', color: 'var(--text-muted)', width: '100%', marginBottom: '10px' }}>Guardado: <b style={{fontSize:'18px', color:'var(--text-main)'}}>${acumulado.toLocaleString()}</b></span>
                                                    
                                                    {/* 💡 BOTONES CONECTADOS */}
                                                    <div className="btn-group-mobile" style={{ display: 'flex', gap: '8px', width: '100%', alignItems: 'center', justifyContent: 'space-between' }}>
                                                        <div style={{ display: 'flex', gap: '5px' }}>
                                                            <button onClick={() => handleManualMeta(meta, 'add')} style={{...btnSmallAccion, color: 'var(--success)', background: 'var(--success-light)', border: '1px dashed var(--success)'}} title="Abonar Manualmente">+ Ingresar Extra</button>
                                                            <button onClick={() => handleManualMeta(meta, 'remove')} style={{...btnSmallAccion, color: 'var(--danger)', background: 'var(--danger-light)', border: '1px dashed var(--danger)'}} title="Retirar Manualmente">- Retirar Fondos</button>
                                                        </div>
                        ...
                        <div style={calendarContainer}>
                            <h4 style={{ margin: '0 0 15px 0', fontSize: '15px', color: 'var(--text-main)' }}><FaCalendarAlt color="var(--primary)" /> Calendario de Metas</h4>
...
const backBtnStyle = { padding: '10px 15px', borderRadius: '12px', border: '1px solid var(--border-color)', cursor: 'pointer', fontWeight: 'bold', display:'flex', alignItems:'center', gap:'8px', backgroundColor: 'var(--bg-tertiary)', color: 'var(--text-main)', transition: '0.2s', fontSize: '14px' };
const contentContainer = { padding: '40px', maxWidth: '1200px', margin: '0 auto', boxSizing: 'border-box' };
const acumuladoCard = { padding: '40px', borderRadius: '30px', textAlign: 'center', marginBottom: '40px', boxShadow: '0 15px 30px rgba(0,123,255,0.2)', transition: '0.3s' };
const gridStyle = { display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: '40px' };
const sectionTitle = { display: 'flex', alignItems: 'center', gap: '10px', fontSize: '18px', marginBottom: '20px', color: 'var(--text-main)', borderBottom: '2px solid var(--border-color)', paddingBottom: '10px', transition: '0.3s' };
...
const progressBg = { width: '100%', height: '12px', backgroundColor: 'var(--bg-tertiary)', borderRadius: '10px', overflow: 'hidden', marginTop: '10px', border: '1px solid var(--border-light)' };
const progressFill = { height: '100%', background: 'var(--primary)', transition: 'width 0.5s ease-out' };
const btnComprar = { background: 'var(--primary)', color: 'white', border: 'none', padding: '10px 15px', borderRadius: '10px', fontSize: '13px', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px' };
const btnDelete = { background: 'var(--danger-light)', color: 'var(--danger)', border: '1px solid var(--danger-light)', padding: '10px', borderRadius: '10px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' };
const badgeProgreso = { fontSize: '12px', color: 'var(--primary)', background: 'var(--primary-light)', padding: '8px 12px', borderRadius: '8px', fontWeight: 'bold', border: '1px solid var(--primary-light)' };
const historialContainer = { background: 'var(--bg-card)', padding: '20px', borderRadius: '20px', border: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', gap: '10px', maxHeight: '400px', overflowY: 'auto', transition: '0.3s' };
const historialItem = { display: 'flex', alignItems: 'center', gap: '10px', padding: '15px', background: 'var(--bg-secondary)', borderRadius: '12px', border: '1px solid var(--border-light)' };
const dateBadge = { background: 'var(--bg-tertiary)', padding: '5px 10px', borderRadius: '8px', fontSize: '11px', fontWeight: 'bold', color: 'var(--text-main)', border: '1px solid var(--border-color)' };
const inputStyle = { flex: 1, padding: '15px', borderRadius: '12px', border: '1px solid var(--border-light)', outline: 'none', background: 'var(--bg-secondary)', color: 'var(--text-main)', fontSize: '15px', boxSizing: 'border-box', fontWeight: 'bold', transition: '0.3s' };
...
const btnSmallAccion = { padding: '8px 12px', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px', fontSize: '12px' };
...
const inputModalStyle = { width: '100%', padding: '15px', borderRadius: '12px', border: '1px solid var(--border-light)', color: 'var(--text-main)', marginBottom: '15px', background: 'var(--bg-secondary)', transition: '0.3s' };
