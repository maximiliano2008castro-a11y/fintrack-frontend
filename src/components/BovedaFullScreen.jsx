...
// 🔴 Añadimos 'saldoCajaFuerte' a las propiedades para poder calcular el Gran Total
const BovedaFullScreen = ({ isOpen, onClose, saldoBoveda, saldoActual, saldoCajaFuerte, pinSeguridad, onTransaction, historial, eventosCalendario, onSaveEvent, onDeleteEvent, mesActual, anioActual }) => {
    ...
    // 🔴 CÁLCULO DEL GRAN TOTAL DE TU DINERO
    ...
    // 🔴 LÓGICA DE CALENDARIO CORREGIDA (PERSPECTIVA LOCAL)
    const checkEventosDia = (dia) => {
        ...
        // Verificamos por la palabra en el historial si el dinero entró o salió
        ...
        <div style={fullScreenStyle}>
            ...
            <div style={headerStyle}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                    ...
                    <h1 style={{ margin: 0, color: '#2f3542', fontSize: '28px', display: 'flex', alignItems: 'center', gap: '10px' }}><FaShieldAlt color="#28a745" /> Bóveda Intocable</h1>
                </div>
            </div>
            ...
            <div style={contentContainer}>
                <div style={cardStyle}>
                    ...
                    <h1 style={balanceStyle}>${saldoBoveda.toLocaleString('es-MX', {minimumFractionDigits: 2})}</h1>
                    ...
                    <div style={actionRow}>
                        <button onClick={handleAdd} style={btnAdd}><FaArrowUp /> Blindar Dinero</button>
                            ...
                            {localHistorial.length === 0 ? ( <p style={{ margin: 0, fontSize: '13px', color: '#a4b0be' }}>Sin movimientos de emergencia.</p> ) : (
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', maxHeight: '180px', overflowY: 'auto' }}>
                                    {localHistorial.map(h => {
                                        // 🔴 LÓGICA DE HISTORIAL CORREGIDA (PERSPECTIVA LOCAL)
...
const cardStyle = { backgroundColor: '#fff', padding: '50px 40px', borderRadius: '35px', boxShadow: '0 20px 50px rgba(0,0,0,0.05)', textAlign: 'center', border: '2px solid #e6f4ea', width: '100%', marginBottom: '30px' };
