...
import { 
    ...
    FaBullseye, FaLockOpen, FaEdit, FaLightbulb, FaPiggyBank, FaLock, FaSyncAlt
} from 'react-icons/fa';
...
const Dashboard = () => {
    ...
    useEffect(() => {
        ...
        // 🛡️ Si el correo no existe en la BD, lo creamos forzosamente para no perder el login
        if (!db[userEmail]) {
            db[userEmail] = { password: '123' }; 
    ...
    useEffect(() => {
        ...
        let tocaCobro = false;
        if (cicloMaestro === 'Semanal') {
            if (hoyDate.getDay().toString() === diaInicioCiclo) tocaCobro = true;
        } else if (cicloMaestro === 'Quincenal') {
            const diaMes = hoyDate.getDate();
            const diaInicioInt = parseInt(diaInicioCiclo);
            if (diaMes === diaInicioInt || diaMes === (diaInicioInt + 15 > 30 ? 15 : diaInicioInt + 15)) tocaCobro = true;
        } else if (cicloMaestro === 'Mensual') {
            if (hoyDate.getDate().toString() === diaInicioCiclo) tocaCobro = true;
        }
        ...
        if (tocaCobro) {
            ...
            nO = nO.filter(n => {
                if (nC[n] && nC[n].frecuencia === 'Imprevisto_Pagado') {
                    delete nC[n]; 
                    return false; 
                }
                return true;
            ...
            setCajonesCorte(pendientes);
            setIngresoPendienteCorte(totalIngresoAAgregar);
            setIsCorteModalOpen(true);
            setUltimaFechaReinicio(fechaStringHoy); 
        }
    ...
    const procesarCorteCajon = (index, accion) => {
        ...
        if (accion === 'gastar') {
            registrarEnHistorial(`${cajon.nombre} (Corte de Ciclo)`, cajon.llenado, 'gasto', saldoActual, saldoActual - cajon.llenado);
            setSaldoActual(p => p - cajon.llenado);
        }
        const nuevos = [...cajonesCorte];
        nuevos[index].procesado = true;
        setCajonesCorte(nuevos);
    };

    const finalizarCorte = () => {
        const montoNuevo = parseFloat(ingresoPendienteCorte) || 0;
        if (montoNuevo > 0) {
            registrarEnHistorial(`¡Inicio de Ciclo! (${cicloMaestro})`, montoNuevo, 'pago', saldoActual, saldoActual + montoNuevo);
            setSaldoActual(p => p + montoNuevo);
        }
        setIsCorteModalOpen(false);
        alert(`✅ ¡Tus cascadas se han reiniciado con éxito para este nuevo ciclo!`);
    };

    // ==========================================
    // 🛡️ MODO BLINDADO: FINALIZAR BIENVENIDA
    // ==========================================
    const handleFinalizarBienvenida = (datos) => {
        let db = JSON.parse(localStorage.getItem('finTrack_DB') || '{}');
        
        // Lo creamos a la fuerza si no existe para que nunca pierda datos
    ...
    // ==========================================
    // 🛡️ MODO BLINDADO: AUTOGUARDADO
    // ==========================================
    useEffect(() => {
        if (!email || isFirstTime || isLoading) return;
        let db = JSON.parse(localStorage.getItem('finTrack_DB') || '{}');
        
        // Lo creamos a la fuerza
        if (!db[email]) db[email] = {}; 
    ...
    const registrarEnHistorial = (nombre, monto, tipo, saldoAnt, saldoNuev) => {
        setHistorial(prev => [{ 
            id: Date.now(), dia: hoy.getDate(), mes: hoy.getMonth(), anio: hoy.getFullYear(), 
            hora: hoy.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }), 
            nombre, monto: parseFloat(monto), tipo,
            saldoAnterior: saldoAnt !== undefined ? saldoAnt : null,
            saldoNuevo: saldoNuev !== undefined ? saldoNuev : null
        }, ...prev]);
    };
    ...
    const borrarCajon = (nombre, llenadoActual = 0) => {
        ...
        if (window.confirm(`¿Seguro que quieres eliminar "${nombre}"?`)) {
            if (llenadoActual > 0 && cajones[nombre].frecuencia !== 'Imprevisto_Pagado') {
                const accion = window.prompt(`Este cajón tiene $${llenadoActual.toLocaleString()} retenidos.\n\nEscribe '1' si ya GASTASTE este dinero (Meta cumplida).\nEscribe '2' si solo cancelas la meta (El dinero volverá a Libre).`);
                if (accion === '1') {
                    registrarEnHistorial(`${nombre} (Completado/Pagado)`, llenadoActual, 'gasto', saldoActual, saldoActual - llenadoActual);
                    setSaldoActual(p => p - llenadoActual);
                } else if (accion !== '2') return alert('❌ Cancelado.');
            }
    ...
    const gastarCapricho = (nombre, monto) => {
        if (window.confirm(`🎉 ¡Felicidades! ¿Confirmas que compraste "${nombre}" por $${monto.toLocaleString()}?\n\nEsto restará el dinero de tu saldo físico y eliminará el cajón.`)) {
            registrarEnHistorial(`${nombre} (Meta Cumplida)`, monto, 'gasto', saldoActual, saldoActual - monto);
    ...
    const handleGuardarNuevoCajon = () => {
        ...
        setCajones(prev => ({ ...prev, [nuevoCajon.nombre]: { monto: parseFloat(nuevoCajon.monto), frecuencia: nuevoCajon.frecuencia } }));
        setOrdenCajones(prev => [...prev, nuevoCajon.nombre]);
        setIsAddModalOpen(false); setNuevoCajon({ nombre: '', monto: '', frecuencia: 'Mensual' });
    };
    ...
    const handleGuardarImprevisto = () => {
        ...
        if (montoImprevisto > saldoActual) return alert('❌ No tienes suficiente dinero físico para cubrir este imprevisto. Tienes que ingresar dinero primero o sacarlo de tus ahorros.');

        const nombreImp = `🚨 Imprevisto: ${imprevistoForm.nombre}`;
        registrarEnHistorial(nombreImp, montoImprevisto, 'gasto', saldoActual, saldoActual - montoImprevisto);
        setSaldoActual(p => p - montoImprevisto);
        setCajones(prev => ({ ...prev, [nombreImp]: { monto: montoImprevisto, frecuencia: 'Imprevisto_Pagado' } }));
        setOrdenCajones(prev => [nombreImp, ...prev]);
        setIsImprevistoModalOpen(false); setImprevistoForm({ nombre: '', monto: '' });
    };

    const agregarCajonDesdeMetas = (nombre, monto, frecuencia) => {
        setCajones(prev => ({ ...prev, [nombre]: { monto, frecuencia } }));
    ...
    const moverCajonArribaAbajo = (index, direccion) => {
        ...
        if (swapIndex >= 0 && swapIndex < nuevoOrden.length) {
            ...
            if (isAdmin(nuevoOrden[index]) === isAdmin(nuevoOrden[swapIndex])) {
                [nuevoOrden[index], nuevoOrden[swapIndex]] = [nuevoOrden[swapIndex], nuevoOrden[index]];
                setOrdenCajones(nuevoOrden);
            }
        }
    };

    const abrirEdicionMonto = (nombreCajon) => {
        if (isAdmin(nombreCajon) || nombreCajon === 'Fondo de Emergencia') { 
            if (window.prompt(`🔒 NIP requerido para editar admin:`) !== pinSeguridad) return alert('❌ NIP Incorrecto.'); 
        }
        setEditMontoForm({ nombre: nombreCajon, monto: cajones[nombreCajon].monto, frecuencia: cajones[nombreCajon].frecuencia });
    ...
    const guardarEdicionMonto = () => {
        ...
        setCajones(prev => ({ ...prev, [editMontoForm.nombre]: { monto: parseFloat(editMontoForm.monto), frecuencia: editMontoForm.frecuencia } }));
        setIsEditMontoOpen(false);
    };

    const abrirReconfiguracion = () => { 
        setConfigForm({ saldoActual: saldoActual, cicloMaestro: cicloMaestro, ingresos: JSON.parse(JSON.stringify(ingresos)) }); 
        setIsProfileOpen(false); setIsReconfigureOpen(true); 
    };
    ...
    const handleGuardarReconfiguracion = () => {
        ...
        setSaldoActual(parseFloat(configForm.saldoActual)); setCicloMaestro(configForm.cicloMaestro); setIngresos(configForm.ingresos);
    ...
    const handleAddMoney = () => {
        const motivo = window.prompt('💰 Motivo del ingreso:'); if (!motivo) return;
        const m = window.prompt('¿Cuánto entró?');
        const montoIngresado = parseFloat(m);
        if (m && !isNaN(m) && montoIngresado > 0) { 
            registrarEnHistorial(motivo, montoIngresado, 'pago', saldoActual, saldoActual + montoIngresado); 
            setSaldoActual(p => p + montoIngresado); 
        }
    };

    const handleRemoveMoney = () => {
        const motivo = window.prompt('💸 Gasto:'); if (!motivo) return;
        const m = window.prompt('¿Cuánto costó?');
        const montoGastado = parseFloat(m);
        if (m && !isNaN(m) && montoGastado > 0) {
            if (montoGastado > saldoActual) return alert('❌ No tienes liquidez.');
            if (window.prompt('🔒 Confirma NIP:') === pinSeguridad) { 
                registrarEnHistorial(motivo, montoGastado, 'gasto', saldoActual, saldoActual - montoGastado); 
                setSaldoActual(p => p - montoGastado); 
    ...
    const abrirEditarPerfil = () => {
        ...
        const pass = db[email] ? db[email].password : '';
        setProfileForm({ nombre: userName, correo: email, fechaNacimiento: fechaNacimiento, password: pass, confirmPassword: pass, pin: '', confirmPin: '' });
        setIsProfileOpen(false); setIsEditProfileOpen(true);
    };

    const handleGuardarPerfil = () => {
        if (!profileForm.nombre) return alert("❌ Nombre vacío.");
        if (profileForm.password && profileForm.password !== profileForm.confirmPassword) return alert("❌ Contraseñas no coinciden.");
        ...
        const db = JSON.parse(localStorage.getItem('finTrack_DB') || '{}');
        if (db[email]) {
            db[email].nombre = profileForm.nombre;
            if (profileForm.password) db[email].password = profileForm.password;
            if (profileForm.pin) db[email].pinSeguridad = profileForm.pin;
            if (profileForm.fechaNacimiento) db[email].fechaNacimiento = profileForm.fechaNacimiento;
            localStorage.setItem('finTrack_DB', JSON.stringify(db));
        }
    ...
    const abrirBoveda = () => {
        ...
        if (window.prompt('🔒 Ingresa NIP para abrir la Bóveda:') === pinSeguridad) setIsBovedaFullScreenOpen(true); else alert('❌ NIP Incorrecto.');
    };

    const abrirCajaFuerte = () => {
        setIsProfileOpen(false);
        if (window.prompt('🔒 Ingresa NIP para abrir Caja Fuerte:') === pinSeguridad) setIsCajaFuerteFullScreenOpen(true); else alert('❌ NIP Incorrecto.');
    };

    const handleCajaFuerteTransaction = (tipo, monto, esExterno = false) => {
        if (tipo === 'add') {
            registrarEnHistorial(esExterno ? 'Ingreso Externo a Caja Fuerte' : 'Ingreso a Caja Fuerte', monto, esExterno ? 'pago' : 'gasto', saldoActual, esExterno ? saldoActual : saldoActual - monto);
            if (!esExterno) setSaldoActual(p => p - monto);
            setCajaFuerte(p => p + monto);
        } else if (tipo === 'withdraw') {
            registrarEnHistorial('Retiro de Caja Fuerte', monto, 'pago', saldoActual, saldoActual + monto);
            setSaldoActual(p => p + monto);
            setCajaFuerte(p => p - monto);
        }
    };
    ...
    const handleBovedaTransaction = (tipo, monto, esExterno = false) => {
        if (tipo === 'add') {
            registrarEnHistorial(esExterno ? 'Blindaje Externo a Bóveda' : 'Blindaje Manual a Bóveda', monto, esExterno ? 'pago' : 'gasto', saldoActual, esExterno ? saldoActual : saldoActual - monto);
            if (!esExterno) setSaldoActual(p => p - monto);
            setBoveda(p => p + monto);
        } else if (tipo === 'withdraw') {
            registrarEnHistorial('Retiro Crítico de Bóveda', monto, 'pago', saldoActual, saldoActual + monto);
            setSaldoActual(p => p + monto);
            setBoveda(p => p - monto);
        }
    };
    ...
    const asegurarFondos = (nombreCajon, montoCajon) => {
        const esEmergencia = nombreCajon === 'Fondo de Emergencia';
        const destino = esEmergencia ? 'Bóveda Intocable' : 'Caja Fuerte';
        if (window.confirm(`¿Asegurar $${montoCajon.toLocaleString()} del cajón "${nombreCajon}" en tu ${destino}?`)) {
            if (!['Gastos Fijos', 'Gastos Variables', 'Ahorro', 'Deuda'].includes(nombreCajon) || esEmergencia) {
                const nC = { ...cajones }; delete nC[nombreCajon]; setCajones(nC);
    ...
    const guardarEvento = () => {
        ...
        setEventosCalendario(isEditing ? eventosCalendario.map(e => e.id === editingId ? { ...e, ...eventForm, monto: parseFloat(eventForm.monto) } : e) : [...eventosCalendario, { id: Date.now(), dia: selectedDay, ...eventForm, monto: parseFloat(eventForm.monto), mesCreacion: mesActual, anioCreacion: anioActual }]);
        cerrarModalEvento();
    };
    
    const eliminarEventoTotal = () => { 
        const ev = eventosCalendario.find(e => e.id === editingId);
        if (!ev) return;
        ...
        if (accion === '1') {
            if (ev.tipo === 'gasto') {
                if (ev.monto > saldoActual) return alert('❌ No tienes saldo físico suficiente para este gasto.');
                registrarEnHistorial(`${ev.nombre}`, ev.monto, 'gasto', saldoActual, saldoActual - ev.monto);
                setSaldoActual(p => p - ev.monto);
            } else {
                registrarEnHistorial(`${ev.nombre}`, ev.monto, 'pago', saldoActual, saldoActual + ev.monto);
                setSaldoActual(p => p + ev.monto);
            }
            setEventosCalendario(eventosCalendario.filter(e => e.id !== editingId)); 
            cerrarModalEvento();
        } else if (accion === '2') {
            setEventosCalendario(eventosCalendario.filter(e => e.id !== editingId)); 
            cerrarModalEvento();
    ...
    ordenCajones.forEach(n => {
        ...
        if (cajones[n].frecuencia === 'Imprevisto_Pagado') {
            estadoCajones.push({ nombre: n, meta: meta, llenado: meta, porcentaje: 100, esSobrante: false });
        } else {
            let llenado = Math.min(Math.max(0, saldoRestante), meta);
            saldoRestante -= llenado;
            estadoCajones.push({ nombre: n, meta, llenado, porcentaje: meta > 0 ? Math.min((llenado / meta) * 100, 100) : 100, esSobrante: false });
        }
    });
    ...
    if (isFirstTime) return <div style={appWrapperStyle}><BienvenidaFullScreen onFinish={handleFinalizarBienvenida} /></div>;
        ...
        <div style={appWrapperStyle}>
            ...
            {isCorteModalOpen && (
                <div style={{...overlayStyle, zIndex: 9999}}>
                    <div style={{...modalCenterStyle, width: '450px', maxHeight: '85vh', overflowY: 'auto', zIndex: 10000}}>
                                ...
                                return (
                                    <div key={i} style={{ padding: '15px', borderRadius: '15px', border: '1px solid #e1e5ee', background: '#f8f9fa' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '15px' }}>
                                            <b style={{ color: '#2f3542' }}>{c.nombre}</b>
                                            <b style={{ color: '#007bff' }}>${c.llenado.toLocaleString()}</b>
                                        </div>
                        ...
                        {cajonesCorte.every(c => c.procesado) && (
                            <div style={{ marginTop: '20px', animation: 'fadeIn 0.3s ease-in' }}>
                                <div style={{ background: '#eef3ff', padding: '20px', borderRadius: '15px', textAlign: 'center', marginBottom: '15px', border: '1px solid #007bff' }}>
                                    ...
                                    <p style={{ margin: '0 0 15px 0', fontSize: '12px', color: '#747d8c' }}>Tus ingresos programados suman esto, pero puedes modificarlo si ganaste más o menos:</p>
                                    <div style={{position: 'relative'}}>
                                        <span style={{ position: 'absolute', left: '15px', top: '50%', transform: 'translateY(-50%)', fontWeight: 'bold', color: '#007bff' }}>$</span>
                                        <input 
                                            type="number" 
                                            value={ingresoPendienteCorte} 
                                            onChange={(e) => setIngresoPendienteCorte(e.target.value)} 
                                            style={{...inputModalStyle, paddingLeft: '35px', margin: 0, fontSize: '20px', fontWeight: 'bold', color: '#007bff', borderColor: '#007bff'}} 
                                        />
                                    </div>
                                </div>
                                <button onClick={finalizarCorte} style={{ ...btnMainAdd, background: '#007bff', width: '100%' }}>
                                    Iniciar Ciclo con ${parseFloat(ingresoPendienteCorte || 0).toLocaleString()}
                                </button>
                            </div>
                        )}
                    </div>
            ...
            {isEditMontoOpen && (
                <div style={{ ...modalCenterStyle, width: '380px' }}>
                    ...
                    <div style={{ position: 'relative', marginBottom: '15px' }}>
                        ...
                        <input type="number" min="0" value={editMontoForm.monto} onChange={(e) => setEditMontoForm({...editMontoForm, monto: e.target.value})} style={{ ...inputModalStyle, paddingLeft: '35px', margin: 0, fontSize: '18px', fontWeight: 'bold' }} />
                    </div>
            ...
            {isEditProfileOpen && (
                <div style={{...modalCenterStyle, width: '420px', maxHeight: '90vh', overflowY: 'auto'}}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                        <h3 style={{ margin: 0, color: '#2f3542', display: 'flex', alignItems: 'center', gap: '8px' }}><FaUserEdit /> Editar Perfil</h3>
                        <FaTimes onClick={() => setIsEditProfileOpen(false)} style={{ cursor: 'pointer', fontSize: '20px', color: '#a4b0be' }} />
                    </div>
                    ...
                    <div style={{ padding: '15px', background: '#f8f9fa', borderRadius: '15px', border: '1px solid #e1e5ee', marginBottom: '20px', marginTop: '10px' }}>
                        ...
                        {profileForm.password && ( <><label style={{...labelModalStyle, color: '#007bff'}}>Confirmar Contraseña</label><input type="password" value={profileForm.confirmPassword} onChange={(e) => setProfileForm({...profileForm, confirmPassword: e.target.value})} style={{...inputModalStyle, border: '1px solid #007bff'}} /></> )}
            ...
            {isReconfigureOpen && (
                <div style={{...modalCenterStyle, width: '450px', maxHeight: '90vh', overflowY: 'auto'}}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                    ...
                    <label style={labelModalStyle}>Saldo Físico Actual</label><input type="number" min="0" value={configForm.saldoActual} onChange={(e) => setConfigForm({...configForm, saldoActual: e.target.value})} style={{ ...inputModalStyle, fontSize: '20px', fontWeight: 'bold' }} />
                    ...
                    {configForm.ingresos.map((ing, i) => (
                        <div key={i} style={{ display: 'flex', gap: '10px', marginBottom: '10px' }}>
                            <input type="number" min="0" value={ing.monto} onChange={(e) => handleIngresoChange(i, 'monto', e.target.value)} style={{...inputModalStyle, flex: 2, margin: 0}} />
            ...
            {isQuickEventOpen && (
                <div style={modalCenterStyle}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                        <div style={badgeDayStyle}>Día {selectedDay}</div>
                        <FaTimes onClick={cerrarModalEvento} style={{ cursor: 'pointer', color: '#a4b0be', fontSize: '20px' }} />
                    </div>
                    <div style={typeSelectorGrid}>
                        <button onClick={() => setEventForm({...eventForm, tipo: 'pago'})} style={{ ...typeBtn, border: eventForm.tipo === 'pago' ? '2px solid #28a745' : '1px solid #e1e5ee', background: eventForm.tipo === 'pago' ? '#e6f4ea' : '#fff' }}>Cobro</button>
                        <button onClick={() => setEventForm({...eventForm, tipo: 'gasto'})} style={{ ...typeBtn, border: eventForm.tipo === 'gasto' ? '2px solid #dc3545' : '1px solid #e1e5ee', background: eventForm.tipo === 'gasto' ? '#fce8e6' : '#fff' }}>Gasto</button>
                    </div>
            ...
            {isAddModalOpen && (
                <div style={modalCenterStyle}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                        <h3 style={{ margin: 0 }}>Nuevo Cajón</h3><FaTimes onClick={() => setIsAddModalOpen(false)} style={{ cursor: 'pointer', color: '#a4b0be' }} />
                    </div>
                    <input type="text" placeholder="Nombre" onChange={(e) => setNuevoCajon({...nuevoCajon, nombre: e.target.value})} style={inputModalStyle} />
                    <div style={{ display: 'flex', gap: '10px' }}>
                        <input type="number" placeholder="Monto" onChange={(e) => setNuevoCajon({...nuevoCajon, monto: e.target.value})} style={{ ...inputModalStyle, flex: 1, margin: 0 }} />
                        <select onChange={(e) => setNuevoCajon({...nuevoCajon, frecuencia: e.target.value})} style={{ ...inputModalStyle, flex: 1, margin: 0 }} defaultValue="Mensual">
                            <option>Único</option><option>Diario</option><option>Semanal</option><option>Quincenal</option><option>Mensual</option><option>Anual</option>
                        </select>
                    </div>
            ...
            {isImprevistoModalOpen && (
                <div style={modalCenterStyle}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                        <h3 style={{ margin: 0, color: '#dc3545', display: 'flex', alignItems: 'center', gap: '8px' }}><FaExclamationTriangle /> Gasto Imprevisto</h3>
                        <FaTimes onClick={() => setIsImprevistoModalOpen(false)} style={{ cursor: 'pointer', color: '#a4b0be' }} />
                    </div>
                    ...
                    <input type="number" placeholder="Costo" value={imprevistoForm.monto} onChange={(e) => setImprevistoForm({...imprevistoForm, monto: e.target.value})} style={inputModalStyle} />
            ...
            <MetasFullScreen 
                isOpen={isMetasOpen} 
                onClose={() => setIsMetasOpen(false)} 
                onAddCajon={agregarCajonDesdeMetas} 
                onReplaceAhorro={reemplazarAhorroAdmin}
                onOpenEmergencia={() => { setIsMetasOpen(false); setIsEmergenciaOpen(true); }}
                gastosFijosBase={cajones['Gastos Fijos']?.monto || 0}
                gastosVariablesBase={cajones['Gastos Variables']?.monto || 0}
                ingresosMensuales={ingresosMensuales} 
                edadUsuario={18} 
                cicloMaestro={cicloMaestro} 
            />

            <BovedaFullScreen
                isOpen={isBovedaFullScreenOpen}
                onClose={() => setIsBovedaFullScreenOpen(false)}
                saldoBoveda={boveda}
                saldoActual={saldoActual}
                saldoCajaFuerte={cajaFuerte}
                pinSeguridad={pinSeguridad}
                onTransaction={handleBovedaTransaction}
                historial={historial}
            ...
            <div style={{ ...profileMenuSidebar, transform: isProfileOpen ? 'translateX(0)' : 'translateX(100%)' }}>
                <div style={{display:'flex', justifyContent:'space-between', alignItems: 'center'}}>
                    <h3 style={{ margin: 0, fontSize: '22px', color: '#2f3542' }}>Menú Principal</h3>
                    <FaTimes onClick={()=>setIsProfileOpen(false)} style={{cursor:'pointer', fontSize: '24px', color: '#a4b0be'}}/>
                </div>
                <div style={{textAlign:'center', margin:'30px 0'}}>
                    <div style={avatarLargeStyle}>{userName.charAt(0).toUpperCase()}</div>
                    <h2 style={{ margin: '15px 0 5px 0', color: '#2f3542' }}>{userName}</h2>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    <button style={{...profileActionBtn, border: '2px solid #007bff', background: '#eef3ff'}} onClick={abrirCajaFuerte}>
                        <FaLock style={{ color: '#007bff', fontSize: '18px' }} /> Caja Fuerte (Ahorros)
                    </button>
                    <button style={{...profileActionBtn, border: '2px solid #28a745', background: '#e6f4ea'}} onClick={abrirBoveda}>
                        <FaShieldAlt style={{ color: '#28a745', fontSize: '18px' }} /> Fondo de Emergencia
                    </button>
                    <button style={profileActionBtn} onClick={() => { setIsProfileOpen(false); setIsHistorialOpen(true); }}>
                        <FaHistory style={{ color: '#10ac84', fontSize: '18px' }} /> Historial Detallado
                    </button>
                    <button style={profileActionBtn} onClick={() => { setIsProfileOpen(false); setIsMetasOpen(true); }}>
                        <FaBullseye style={{ color: '#dc3545', fontSize: '18px' }} /> Mis Metas y Herramientas
                    </button>
                    <hr style={{ border: '0', borderTop: '1px solid #e1e5ee', margin: '10px 0' }} />
                    <button style={profileActionBtn} onClick={abrirEditarPerfil}>
                        <FaUserEdit style={{ color: '#28a745', fontSize: '18px' }} /> Editar Perfil y Contraseña
                    </button>
                    <button style={profileActionBtn} onClick={abrirReconfiguracion}>
                        <FaCog style={{ color: '#007bff', fontSize: '18px' }} /> Reconfigurar Finanzas Base
                    </button>
                </div>
            ...
            <aside style={sidebarStyle}>
                <div style={{ marginBottom: '30px' }}>
                    <h2 style={{ margin: '0', color: '#007bff', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '8px' }}><FaWallet /> FinTrack</h2>
                    <span style={{ fontSize: '12px', background: '#eef3ff', color: '#007bff', padding: '4px 12px', borderRadius: '20px', fontWeight: 'bold', display: 'inline-block', marginTop:'8px' }}>Ciclo: {cicloMaestro}</span>
                </div>
                <hr style={{ border: '0', borderTop: '1px solid #e1e5ee', marginBottom: '20px' }} />

                <div style={calendarContainer}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
                        <span style={{ fontSize: '14px', fontWeight: 'bold', color: '#2f3542' }}><FaCalendarAlt color="#007bff" /> {['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre'][mesActual]}</span>
                        <span onClick={() => setIsFullScreenCalendar(true)} style={{fontSize:'11px', color:'#007bff', cursor:'pointer', fontWeight:'bold', background:'#eef3ff', padding:'3px 8px', borderRadius:'10px'}}>Ver Todo</span>
                    </div>
                    ...
                    <div style={gridCalendario}>
                        {(() => {
                            ...
                            return dias.map((dia, i) => {
                                ...
                                if (tienePago && tieneGasto) { 
                                    bgColor = 'linear-gradient(135deg, #28a745 50%, #dc3545 50%)'; textColor = 'white'; 
                                } else if (tienePago) { 
                                    bgColor = '#28a745'; textColor = 'white'; 
                                } else if (tieneGasto) { 
                                    bgColor = '#dc3545'; textColor = 'white'; 
                                } else if (esHoy) { 
                                    bgColor = '#007bff'; textColor = 'white'; 
                                }

                                if (esHoy && (tienePago || tieneGasto)) {
                                    boxShadowStyle = 'inset 0 0 0 2px #fff, inset 0 0 0 4px #007bff, 0 3px 10px rgba(0,0,0,0.15)';
                                } else if (esHoy) {
                                    boxShadowStyle = '0 3px 10px rgba(0,123,255,0.4)';
                                } else if (tienePago || tieneGasto) {
                                    boxShadowStyle = '0 3px 10px rgba(0,0,0,0.15)';
                                }
                                
                                return <div key={i} onClick={() => { if(dia) handleDayClick(dia); }} style={{...celdaDia, background: bgColor, color: textColor, fontWeight: (tienePago || tieneGasto || esHoy) ? 'bold' : 'normal', cursor: dia ? 'pointer' : 'default', boxShadow: boxShadowStyle}} title={dia ? "Clic para ver o añadir alerta" : ""}>{dia || ''}</div>;
                            });
                        })()}
                    </div>
                </div>
                ...
                <div style={{ ...remindersContainer, marginBottom: '20px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
                        <h4 style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px', margin: 0, color: '#2f3542' }}><FaHistory color="#007bff" /> Últimos Movimientos</h4>
                        {historial.length > 0 && <FaTrash onClick={limpiarHistorial} style={{ cursor: 'pointer', color: '#dc3545', fontSize: '12px' }} title="Vaciar Historial" />}
                    </div>
                    {historial.length === 0 ? ( <p style={{ margin: 0, fontSize: '12px', color: '#a4b0be' }}>Registra tus ingresos y gastos para ver el historial.</p> ) : (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                            {historial.slice(0, 3).map(h => <div key={h.id} style={miniAlertCard} title={h.hora}><span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontSize: '13px' }}>{h.nombre}</span><span style={{ fontWeight: 'bold', fontSize: '13px', color: h.tipo === 'pago' ? '#28a745' : '#dc3545' }}>{h.tipo === 'pago' ? '+' : '-'}${h.monto}</span></div>)}
                        </div>
                    )}
                </div>
                ...
                <div style={remindersContainer}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
                        <h4 style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px', margin: 0, color: '#2f3542' }}><FaBell color="#ffc107" /> Próximas Alertas</h4>
                        <div onClick={() => handleDayClick(hoy.getDate())} style={{cursor:'pointer', color:'#fff', background:'#007bff', padding:'4px 8px', borderRadius:'8px', fontSize:'11px', fontWeight:'bold', display:'flex', alignItems:'center', gap:'5px'}}><FaPlus/> Nueva</div>
                    </div>
                    {eventosCalendario.length === 0 ? ( <p style={{ margin: 0, fontSize: '12px', color: '#a4b0be' }}>No tienes alertas pendientes.</p> ) : (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                            {eventosCalendario.sort((a,b)=>a.dia-b.dia).slice(0, 4).map(e => <div key={e.id} style={{...miniAlertCard, cursor: 'pointer'}} onClick={() => handleEditEvent(e)} title="Clic para editar o borrar"><span style={miniDayBadge}>{e.dia}</span><span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontSize: '13px' }}>{e.nombre}</span><span style={{ fontWeight: 'bold', fontSize: '13px', color: e.tipo === 'pago' ? '#28a745' : '#dc3545' }}>${e.monto}</span></div>)}
                        </div>
                    )}
                </div>
            </aside>
            ...
            <main style={mainContentStyle}>
                <div style={mainContainerMaxWidth}>
                    <header style={headerStyle}>
                        <div><h1 style={{ margin: 0, fontSize: '32px', color: '#2f3542' }}>Dashboard 📊</h1><p style={{ margin: '5px 0 0 0', color: '#747d8c', fontSize:'15px' }}>Todo tu dinero en automático.</p></div>
                    ...
                    <div style={mainCardStyle}>
                        ...
                        <h2 style={{ fontSize: '64px', margin: '15px 0 5px 0', fontWeight: 'bold', color: '#4cd137', textShadow: '0 2px 10px rgba(76, 209, 55, 0.2)' }}>${disponibleLibre.toLocaleString('es-MX', { minimumFractionDigits: 2 })}</h2>
                    ...
                    {sobranteCiclo > 0 && mostrarBannerSobrante && (
                        <div style={{...alertSuccessStyle, display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                            <div style={{ display: 'flex', alignItems: 'center' }}>
                                <FaLightbulb style={{ color: '#ff9f43', fontSize:'24px', marginRight:'15px' }} /> 
                                <div style={{color:'#856404', fontSize: '15px'}}>¡Presupuesto Sano! 📈 Tu configuración genera un margen estimado de <strong>${sobranteCiclo.toLocaleString('es-MX', { minimumFractionDigits: 2 })}</strong> a tu favor por ciclo.</div>
                            </div>
                            <FaTimes onClick={() => setMostrarBannerSobrante(false)} style={{ cursor: 'pointer', color: '#856404', fontSize: '18px' }} />
                        </div>
                    )}

                    {sobranteCiclo < 0 && mostrarBannerSobrante && (
                        <div style={{...alertSuccessStyle, backgroundColor: '#ffebee', borderLeft: '8px solid #dc3545', display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                            <div style={{ display: 'flex', alignItems: 'center' }}>
                                <FaExclamationTriangle style={{ color: '#dc3545', fontSize:'24px', marginRight:'15px' }} /> 
                                <div style={{color:'#c0392b', fontSize: '15px'}}>⚠️ Cuidado: Tus metas superan a tus ingresos. Tienes un déficit de <strong>${Math.abs(sobranteCiclo).toLocaleString('es-MX', { minimumFractionDigits: 2 })}</strong> por ciclo.</div>
                            </div>
                            <FaTimes onClick={() => setMostrarBannerSobrante(false)} style={{ cursor: 'pointer', color: '#c0392b', fontSize: '18px' }} />
                        </div>
                    )}
                        ...
                        {estadoCajones.map((cajon, index) => {
                            ...
                            return (
                                <div key={cajon.nombre} style={cascadeCard}>
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                                            {isEditingCajones && (
                                                <div style={{ display: 'flex', gap: '5px' }}>
                                                    <button onClick={() => moverCajonArribaAbajo(realIndex, 'up')} disabled={!canMoveUp || cajon.nombre === 'Deuda'} style={{...arrowBtn, opacity: (!canMoveUp || cajon.nombre === 'Deuda') ? 0.3 : 1}}><FaChevronUp /></button>
                                                    <button onClick={() => moverCajonArribaAbajo(realIndex, 'down')} disabled={!canMoveDown || cajon.nombre === 'Deuda'} style={{...arrowBtn, opacity: (!canMoveDown || cajon.nombre === 'Deuda') ? 0.3 : 1}}><FaChevronDown /></button>
                                                    <button onClick={() => abrirEdicionMonto(cajon.nombre)} style={{...arrowBtn, color: '#ff9f43', marginLeft: '5px'}} title="Editar Meta"><FaEdit /></button>
                                                    {!isAdmin(cajon.nombre) && <button onClick={() => borrarCajon(cajon.nombre, cajon.llenado)} style={{...arrowBtn, color: '#dc3545', marginLeft: '5px'}}><FaTrash /></button>}
                                                </div>
                                            )}
                                    ...
                                    {isFull && !['Gastos Fijos', 'Gastos Variables', 'Deuda'].includes(cajon.nombre) && !cajon.nombre.includes('Capricho:') && (
                                    ...
                                    {isFull && cajon.nombre.includes('Capricho:') && (
                                        <button onClick={() => gastarCapricho(cajon.nombre, cajon.llenado)} style={{ width: '100%', background: '#fce3ed', color: '#e83e8c', border: `1px dashed #e83e8c`, padding: '10px', borderRadius: '10px', marginTop: '15px', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
                                            <FaCheckCircle /> ¡Capricho Comprado! (Gastar y Borrar)
                                        </button>
                                    )}
                                </div>
                            );
                        })}
                        {estadoCajones.find(c => c.nombre === 'Libre') && (
                            <div style={{...cascadeCard, border: '2px solid #007bff'}}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}><span style={{ fontSize: '16px', fontWeight: 'bold', color: '#007bff' }}>Cajón Libre (Sobrante final)</span><span style={{ fontSize: '16px', fontWeight: 'bold', color: '#007bff' }}>${estadoCajones.find(c => c.nombre === 'Libre').llenado.toLocaleString()}</span></div>
                                <div style={progressBg}><div style={{ ...progressFill, width: `100%`, backgroundColor: '#007bff' }}></div></div>
                            </div>
                        )}
                    </div>
                    ...
                    <div style={cajonesGridStyle}>
                        {estadoCajones.map((cajon, index) => {
                            if (cajon.nombre === 'Libre') return null;
                            const isFull = cajon.llenado >= cajon.meta;
                            const isImp = cajon.nombre.includes('🚨');
                            return (
                    ...
                    pasos={[
                        {
                            ...
                            contenido: "Aquí verás todo tu dinero líquido. Ingresa tu saldo y la Cascada lo repartirá automáticamente llenando tus cajones según su prioridad.",
                            consejo: "Coloca siempre las Deudas y los Gastos Fijos hasta arriba. Que el sistema los pague por ti primero."
                        },
                        {
                            titulo: "El Poder del Sobrante 💸",
                            contenido: "Si te sobra dinero al final del ciclo (Cajón Libre), significa que tu estrategia funcionó y gastaste menos de lo que ganaste.",
                            consejo: "¡No dejes ese dinero ahí! Mándalo a tu Caja Fuerte (Ahorros) para que no te lo gastes por accidente."
                        },
                        {
                            titulo: "Corte de Ciclo 🔄",
                            contenido: "Cuando llegue tu Día de Pago, la app te pedirá hacer cuentas de los cajones llenos para saber qué gastaste y qué te sobró.",
                            consejo: "El sistema inyectará tu nuevo sueldo automáticamente. Solo mantén tus fuentes de ingreso actualizadas."
                        }
                    ]}
                />
            </main>
...
const calendarContainer = { backgroundColor: '#f8f9fa', padding: '20px', borderRadius: '25px', marginBottom: '30px', border: '1px solid #e1e5ee', transition: 'box-shadow 0.2s' };
...
const miniDayBadge = { background: '#e1e5ee', padding: '4px 8px', borderRadius: '8px', fontWeight: 'bold', fontSize: '12px', color: '#2f3542' };
